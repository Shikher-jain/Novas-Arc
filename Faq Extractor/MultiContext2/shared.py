...
def advanced_system_prompt_generator(
    ...
    *,
    ...
    # Keep ALL detected values comma-separated instead of repeating blocks per value
    topics_csv_all = ", ".join([t for t in topics if t])
    personas_csv_all = ", ".join([p for p in personas if p])
    intent_value = detect_intent(question)
    tone_value = tone
    prompt_parts = []
    # Use only the PRIMARY persona for the lead line to avoid repetition
    primary_persona = personas[0] if personas else "assistant"
    key = (primary_persona, tone)
    if key in prompt_template_map:
        lead = prompt_template_map[key]
    else:
        lead = (
            f"You are a {primary_persona} with a {tone} tone. "
            f"Provide clear, accurate, and helpful answers to user questions."
        )
    prompt_parts.append(lead)
    # Append single consolidated context lines
    if topics_csv_all:
        prompt_parts.append(f"Domains: {topics_csv_all}.")
    if personas_csv_all:
        prompt_parts.append(f"Personas: {personas_csv_all}.")
    prompt_parts.append(f"Intent: {intent_value}, Tone: {tone_value}.")
