...
def validate_fine_tuned_model(model_id, user_prompt, platform="corporate_website", touchpoint="support_page", age_group="millennial", answer_style: str = "medium", strict_mode: bool = False):
    """
    ...
    try:
        ...
        # (Removed legacy: universal_prompt_analysis_and_generation)
        sentiment, keywords = analyze_content(user_prompt)
        
    # STEP 2: Multi-dimensional analysis
    # Use sentiment dict for tone detection (function expects sentiment scores)
        ...
        # ============ ADVANCED SYSTEM PROMPT ONLY ============
        keywords_list = list(keywords)
        context = domain  # Use detected domain as context

        final_system_prompt = advanced_system_prompt_generator(
            user_prompt,
            "",  # No answer at runtime; used for topic/persona detection
            context
        )
        ...
        if strict_mode:
            ...
            guidance = get_generation_instructions(answer_style, age_group) + " Start directly with the answer; do not repeat the question or these instructions."

        # Explicitly add an Instructions and Guidelines header block
...
# (Removed save_model_id and select_training_file – not used in chat runtime)
...
def ensemble_ask(models, user_prompt, platform="corporate_website", touchpoint="support_page", answer_style: str = "medium", strict_mode: bool = False):
    """
...
def main():
    ...
    while True:
        ...
        context.system_prompt = advanced_system_prompt_generator(
            user_prompt,
            "",
            context.domain[0] if getattr(context, 'domain', None) else None,
        )
    ...
    # (Legacy universal prompt analysis removed)
