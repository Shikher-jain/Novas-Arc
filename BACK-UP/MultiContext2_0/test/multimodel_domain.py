...
def validate_fine_tuned_model(model_id, user_prompt, platform="corporate_website", touchpoint="support_page"):
    ...
    try:
        ...
        # STEP 6: Create Domain × Tone × Intent system prompt using the unified generator
        # Include platform and touchpoint context in the summary for better steering
        context_summary = f"Platform: {platform} | Touchpoint: {touchpoint} | Detected domain: {domain}"
        final_system_prompt = generate_system_prompt(
            domain=industry,
            tone=platform_tone,
...
def main():
    ...
    while True:
        ...
        context.system_prompt = generate_system_prompt(context=context, mode="minimal")
