...
def analyze_content(user_message):
    ...
    Returns sentiment score and list of keywords.
    """
    sia = SentimentIntensityAnalyzer()
    sentiment = sia.polarity_scores(user_message)
    tokens = word_tokenize(user_message)
    keywords = [w for w in tokens if w.isalpha() and len(w) > 3]
...
def detect_context(url: str, html: str) -> Tuple[str, str]:
    ...
    # Get the context with the highest score
    if context_scores:
        best_context = max(context_scores.items(), key=lambda x: x[1])
        if best_context[1] > 0:  # If we found any matches
            if best_context[0] == "support":
                return "support", "You are a customer support assistant helping users with their questions and issues."
            elif best_context[0] == "technical":
                return "technical", "You are a technical support specialist providing detailed technical information and troubleshooting."
            elif best_context[0] == "sales":
                return "sales", "You are a sales assistant helping customers find the right products and complete their purchases."
            elif best_context[0] == "booking":
                return "booking", "You are a booking assistant helping users schedule and manage appointments."
    
    # Default context
    return "general", "You are a helpful assistant providing information about various topics."
...
def main():
    ...
    def generate_system_prompt(question, answer, context=None):
        """
        Generalized, robust system prompt generator using NLP and context.
        """
        # Analyze both question and answer for sentiment and keywords
        try:
            q_sentiment, q_keywords = analyze_content(question)
        except Exception as e:
            logging.warning(f"NLP analysis failed for question: {question}. Error: {e}")
            q_sentiment, q_keywords = {'compound': 0}, []
        try:
            a_sentiment, a_keywords = analyze_content(answer)
        except Exception as e:
            logging.warning(f"NLP analysis failed for answer: {answer}. Error: {e}")
        ...
        # Topic detection: match keywords to TOPIC_TONE_MAP, fallback to context
        topic = None
        for key in TOPIC_TONE_MAP:
        ...
        # Tone detection: use average sentiment from question and answer
        ...
        # Persona detection: basic rules, can be extended
        persona = "helpful assistant"
        if context == "technical" or "api" in all_keywords or "developer" in all_keywords:
            persona = "technical expert"
        elif context == "sales" or "pricing" in all_keywords or "product" in all_keywords:
            persona = "sales assistant"
        elif context == "support" or "customer" in all_keywords or "support" in all_keywords:
            persona = "customer support agent"
        elif context == "booking" or "schedule" in all_keywords or "booking" in all_keywords:
            persona = "booking assistant"

        # Compose prompt
        system_prompt = (
            f"You are a {persona} with a some {tone} tone. "
    ...
    for context, data in context_faqs.items():
        ...
        for faq in faqs:
            ...
            if q and a and len(a) >= min_len:
                system_prompt = generate_system_prompt(q, a, context)
                record = {
                    "messages": [
        ...
        logging.info(f"Processed {len(faqs)} {context} FAQs")
