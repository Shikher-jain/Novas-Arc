...
from shared_config import (
    ...
    build_system_prompt,
    compose_domain_tone_intent_prompt,
    generate_adaptive_system_prompt,
    evaluate_response,
...
def get_system_prompt(domain, tone, intent, persona=None, context_summary=None):
    """
    Wrapper to get the system prompt using centralized logic from shared_config.py.
    """
    from shared_config import generate_merged_system_prompt
    return generate_merged_system_prompt(domain, tone, intent, persona, "normal", "millennial", True)

def get_fallback_prompt(tone, intent, domain):
    """
    Wrapper to get the fallback prompt using centralized logic.
    """
    return generate_adaptive_system_prompt(tone, intent, domain)
...
def validate_fine_tuned_model(model_id, user_prompt, platform="corporate_website", touchpoint="support_page", age_group="millennial"):
    """
    ...
    try:
        ...
        composite_system_prompt += get_system_prompt(domain, tone, intent, persona=None, context_summary=None)
        ...
        response = openai.chat.completions.create(
            model=model_id,
            messages=[
                {"role": "system", "content": final_system_prompt+ " make sure there is no repetion in the response."},
                {"role": "user", "content": user_prompt + (
                    " Please provide a comprehensive and detailed answer. "
                    "If the solution involves multiple steps, explain each step clearly and thoroughly, breaking down the process in a structured, step-by-step manner and including all necessary details and instructions for clarity. "
                    "I want full page content, mirror the user's energy in your response. "
                    f"Adapt your response for the user's age group: {age_group}. "
                    "If the answer is not found in your training data or is outside your domain, you may use your general or base model knowledge to answer naturally.Every respone must be at least 300 to 400 words."
                )}
            ],
...
def ensemble_ask(models, user_prompt, platform="corporate_website", touchpoint="support_page"):
    ...
    def _call_one(key_mid):
        ...
        r = validate_fine_tuned_model(model_id, user_prompt, platform=platform, touchpoint=touchpoint, age_group=age_group)
